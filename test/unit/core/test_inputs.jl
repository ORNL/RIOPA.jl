...
@testset "cmdline" begin
    ...
    @test_throws ArgParseError("unknown command: hello-fail") RIOPA.parse_inputs(
        ["hello-fail"],
        error_handler = ArgParse.debug_handler,
    )

    @test_throws ArgParseError("unrecognized option --hey") RIOPA.parse_inputs(
        ["--hey"],
        error_handler = ArgParse.debug_handler,
    )
end
